        ...
        try {
            ...
            for(int i=1; i<height-1; i++){
                ...
                for(int j=1; j<width-1; j++) {
                    ...
                    red_pnym = 90 * Math.exp(-(i * i + j * j) / (2 * red_pnym * red_pnym));
                    green_pnym = 90 * Math.exp(-(i * i + j * j) / (2 * green_pnym * green_pnym));
                    blue_pnym = 90 * Math.exp(-(i * i + j * j) / (2 * blue_pnym * blue_pnym));
